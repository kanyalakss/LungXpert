...
def predict_image(img_array):
    ...
    if confidence < 64:
        return "ไม่สามารถจำแนกโรคได้", predictions[0], confidence
    else:
        return class_labels[class_idx], predictions[0], confidence
...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            result, predictions, confidence = predict_image(img_array)
            ...
            return jsonify(result=result, final_img=final_img_base64, confidence=confidence)
...
def save_data():
    ...
    try:
        ...
        prediction_result = session.get('result')
            ...
            db["patient_history"].insert_one(
                {
                    ...
                    "prediction_result": prediction_result,  # Save prediction result
                }
            )
